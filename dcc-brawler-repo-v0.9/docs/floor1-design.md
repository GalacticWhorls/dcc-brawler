...
- **Vine Creeper** *(later, infrequent)* — rooted plant; ranged lash **roots** Carl in
  ...
  wave anchor.
- **Small Lizard** *(later)* — the Juicer's minions; small, fast, bipedal.
...
### The Hoarder — spawner siege *(v0.10)*
...
### The Juicer — roided gym lizard *(v0.11+)*
...
→ Hoarder's lair *(v0.10)* → approach + Juicer *(v0.11+)* → Ball of Swine *(later)*.

## Build order (one playtested increment per version; never over-reach)
1. **v0.7** — Rot Sticker + Cockroach mobs; Enc4/Enc5 approach encounters. *No Hoarder.*
2. **v0.8** — Readability & mix tuning pass (breakable scatter, encounter mix,
   roach/rat sprites). No new systems.
3. **v0.9** — Arcade HUD reorg, multiplayer-ready: per-player panels across the top
   (score in each panel), boss bar along the bottom. Player list architected for up to
   4; one player wired today. No gameplay change.
4. **v0.10** — The Hoarder (lock-in door, spawner, smashable piles, swell→poison cloud,
   huge HP).
5. **v0.11+** — Small Lizard mob + Juicer approach; then the Juicer.
6. **later** — Ball of Swine boss; then playable characters (Donut/Florin/Elle), one
   at a time, so each new character debuts against a finished floor.
