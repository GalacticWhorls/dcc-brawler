...
### The Hoarder — spawner siege *(next)*
...
### The Juicer — roided gym lizard *(later)*
...
→ Hoarder's lair *(next)* → approach + Juicer *(later)* → Ball of Swine *(later)*.
...
- **Next** — The Hoarder (lock-in door, spawner, smashable piles, swell→poison cloud, huge HP)
- **After** — Small Lizard mob + Juicer approach; then the Juicer
