...
### The Hoarder — spawner siege *(v0.8)*
...
### The Juicer — roided gym lizard *(v0.9+)*
...
→ Enc3 Goblin Dozer *(built)* → **Enc4/Enc5 roach+sticker infestation ramp (v0.7)**
→ Hoarder's lair *(v0.8)* → approach + Juicer *(v0.9+)* → Ball of Swine *(later)*.
...
2. **v0.8** — The Hoarder (spawner, smashable piles, swell→poison cloud, huge HP).
3. **v0.9+** — Small Lizard mob + Juicer approach; then the Juicer.
4. **later** — Ball of Swine boss; then playable characters (Donut/Florin/Elle), one
