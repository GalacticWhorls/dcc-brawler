...
check("The Hoarder floods (capped), poisons, and clears the lair on death", () => {
  game.reset();
  const hIdx = game.encounters.findIndex(e => e.waves.some(w => w.some(s => s.type === "the_hoarder")));
  const enc = game.encounters[hIdx];
  game.encIndex = hIdx;
...
// =====================================================================
// CHECK 9 — Special (Daddy Tax): consumes meter, damages + stuns in radius
// =====================================================================
check("Special consumes meter and damages + stuns nearby enemies", () => {
  game.reset();
  game.player.x = 1000; game.player.y = 415; game.player.meter = 100;
  const e = makeEnemy("goblin", 1050, 415); e.entering = false;
  const far = makeEnemy("goblin", 1600, 415); far.entering = false;
  game.enemies = [e, far];
  const hp0 = e.hp, farHp0 = far.hp;
  game.player.startSpecial();
  assert(game.player.meter === 0, "special did not consume meter");
  assert(e.hp < hp0, "special dealt no damage to the near enemy");
  assert(e.hitstun >= 0.8, "special did not stun the near enemy");
  assert(far.hp === farHp0, "special hit an enemy outside its radius");
  return "meter spent, near enemy damaged + stunned, far enemy untouched";
});

// =====================================================================
// CHECK 10 — Gym Lizard tongue reaches past normal melee range
// =====================================================================
check("Gym Lizard tongue lash has extended reach", () => {
  game.reset();
  game.player.x = 1000; game.player.y = 415; game.player.invuln = 0; game.player.lives = 9;
  const liz = makeEnemy("small_lizard", 1078, 415); liz.entering = false;   // ~78px: beyond a goblin's 58 reach
  game.enemies = [liz];
  const startHp = game.player.hp;
  for (let i = 0; i < 260; i++) { game.player.lives = 9; update(dt); assert(isFinite(liz.x), "lizard NaN"); }
  assert(game.player.hp < startHp, "Gym Lizard never landed a tongue hit from range");
  return "lizard poked the player from tongue range";
});
